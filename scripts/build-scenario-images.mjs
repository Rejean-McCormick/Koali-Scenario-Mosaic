...
function scenarioNumber(name) {
  ...
  return number >= 1 && number <= SCENARIO_COUNT ? number : null;
}
...
for (const name of sourceEntries) {
  ...
  if (number === null) {
    errors.push(`${name}: expected SCN-001.png … SCN-${String(SCENARIO_COUNT).padStart(3, '0')}.png`);
    continue;
  }
...
for (const name of publicEntries) {
  ...
  if (!match) continue;
  const number = Number(match[1]);
  if (number < 1 || number > SCENARIO_COUNT) continue;
  const candidates = publicByScenario.get(number) ?? [];
  candidates.push({ name, path: path.join(OUTPUT_DIR, name) });
...
async function choosePublicFallback(number) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // validate-images.py reports malformed outputs later if they are still relevant.
    }
  }

  // Production/Netlify fallback: if source masters are intentionally absent,
  // use the largest usable public PNG as the input and regenerate 418/627.
...
for (let number = 1; number <= SCENARIO_COUNT; number += 1) {
  ...
  if (inputPath) {
    ...
    if (!fallback) continue; // V2 family SVG fallback remains active.
...
async function buildOne({ name, inputPath }) {
  // Read first so a public default can safely be both fallback input and output.
...
if (jobs.length === 0) {
  console.log('Scenario images: no final PNG masters/public fallbacks found; V2 family SVG fallbacks remain active.');
} else {
